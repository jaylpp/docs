...
```bash cURL
curl -X GET https://api.paperpanda.cn/api/v1/billing/balance \
...
url = "https://api.paperpanda.cn/api/v1/billing/balance"
headers = {
    "Authorization": "Bearer ak_xxxxxxxxxxxxxxxx"
}
...
const getBalance = async () => {
  try {
    const response = await axios.get(
      'https://api.paperpanda.cn/api/v1/billing/balance',
      {
        headers: {
          'Authorization': 'Bearer ak_xxxxxxxxxxxxxxxx'
        }
...
public class GetBalance {
    public static void main(String[] args) throws Exception {
        ...
        String url = "https://api.paperpanda.cn/api/v1/billing/balance";
...
<CardGroup cols={2}>
  <Card title="充值记录" icon="credit-card" href="/api-reference/billing/recharge">
    查看充值历史记录
  </Card>
  <Card title="消费记录" icon="receipt" href="/api-reference/billing/consumption">
    查看消费历史记录
  </Card>
  <Card title="AIGC改写" icon="wand-magic-sparkles" href="/api-reference/aigc/rewrite">
    使用余额进行文本改写
  </Card>
  <Card title="API密钥管理" icon="key" href="/api-reference/api-keys/list">
    管理您的API密钥
  </Card>
</CardGroup>
...
<AccordionGroup>
  ...
  <Accordion title="如何计算可用余额能做多少次改写？">
    ...
    price_per_1000 = 10   # 每千字价格（根据实际服务类型）
...
- 📚 完整文档：https://api.paperpanda.cn/docs
- 💬 在线客服：平台右下角
